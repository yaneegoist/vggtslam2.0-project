...
class Solver:
    ...
    def _run_predictions_da3(
        ...
        """
        Run DA3 with its native preprocessing and convert
        its outputs to the VGGT-SLAM prediction format.

        DA3 camera extrinsics are used directly as OpenCV
        world-to-camera transformations.
        """

        if max_loops != 0:
            raise RuntimeError(
                "DA3 currently supports only "
                "--max_loops 0"
            )
        ...
        if processed_images.shape != expected_image_shape:
            raise RuntimeError(
                "DA3 processed image shape mismatch: "
                f"{processed_images.shape}, "
                f"expected {expected_image_shape}"
            )

        if confidence.shape != depth.shape:
            raise RuntimeError(
                "DA3 depth and confidence shapes differ: "
        ...
        if extrinsic.shape != (
            ...
            raise RuntimeError(
                "Unexpected DA3 extrinsic shape: "
                f"{extrinsic.shape}"
            )
        ...
        if intrinsic.shape != (
            ...
            raise RuntimeError(
                "Unexpected DA3 intrinsic shape: "
                f"{intrinsic.shape}"
            )
        ...
        for name, value in arrays_to_check.items():
            if not np.isfinite(value).all():
                raise RuntimeError(
                    f"DA3 {name} contains NaN or Inf"
                )

        if np.any(depth <= 0):
            raise RuntimeError(
                "DA3 returned non-positive depth"
            )

        images = torch.from_numpy(
            processed_images.copy()
        )
        ...
        with self.clip_timer:
            if (
                ...
                new_submap.set_all_semantic_vectors(
                    image_embs
                )
        ...
        print(
            "DA3 output shapes:",
            {
                ...
                "confidence": tuple(
                    predictions["depth_conf"].shape
                ),
                "extrinsic": tuple(
                    predictions["extrinsic"].shape
                ),
                "intrinsic": tuple(
                    predictions["intrinsic"].shape
                ),
            },
        )
    ...
    def run_predictions(
        ...
        if getattr(
            ...
            return self._run_predictions_da3(
                image_names,
                model,
                max_loops,
                clip_model,
        ...
        if max_loops > 0:
            if self.image_retrieval is None:
                raise RuntimeError(
                    "Loop closure retrieval is not initialized"
                )

            detected_loops = (
                self.image_retrieval.find_loop_closures(
                    self.map,
                    new_submap,
